...
@section('view_descrip')
	Buscar farmacias para ver los inventarios
@endsection
@section('content')
...
@section('script')
	<script>
		$(function(){
			...
			$('#form_buscar').submit(function(e) {
				...
				.done(function(data) {
					...
					$.grep(data,function(i,e) {
						rows+= '<tr><td>'+i.nombre_farmacia+'</td><td>'+i.director+'</td><td>'+i.director_number+'</td><td>'+i.director_email+'</td><td>'+i.cantidad+'</td><td class="letras-medianas"><a href="#" class="ver_inventario" target="" user_id='+i.user_id+'><i class="fa fa-external-link-square"></i></a></td></tr>'
					})
			...
			$('#table').children('tbody').on('click','tr td .ver_inventario',function(e){
				e.preventDefault()

				let id = e.currentTarget.getAttribute('user_id'),
					route = "{{ url('admin/stocktaking/pharmacy/view') }}"+"/"+id

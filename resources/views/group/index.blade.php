...
@section('content')
                ...
                <div class="panel-heading">
                    <div class="row no-gutters">
                        ...
                        <div class="col-md-9 col-sm-9 text-right">
                            <div class="huge"></div>
                            <div>Grupos</div>
                            <div style="font-size: 2em;" id="total_registros">{{count($group)}}</div>
                        </div>
                    </div>
                </div>
                <a href="#">
                    ...
                    @foreach($group as $row)
                        <tr>
                            ...
                            @if(Auth::user()->authorized())
                                <td>
                                    <a href="{{ url('group/'.$row->id.'/edit') }}" class="letras-medianas" title="Editar"><i class="fa fa-edit fa-1x"></i></a>
    ...
    <form action="{{ url('group/'.':USER') }}" id="form_eliminar" method="POST">
        {{csrf_field()}}
        {{method_field('DELETE')}}
    </form>
@endsection
